...
void WeatherWidget::fetchForecast(const QString &cityName, const QString &apiKey)
{
    ...
    connect(coordReply, &QNetworkReply::finished, this, [this, coordReply, apiKey]() {
            ...
            connect(reply, &QNetworkReply::finished, this, [this, reply]() {
                if (reply->error() == QNetworkReply::NoError)
                {
                    ...
                    for (const QJsonValue &value : forecastList)
                    {
                        ...
                        dataMap["date"] = dateTime.date().toString("MM-dd");
                        dataMap["time"] = dateTime.time().toString("HH:mm");
                        dataMap["temp"] = obj["main"].toObject()["temp"].toDouble();
                        dataMap["description"] = obj["weather"].toArray().first().toObject()["description"].toString();
                        dataMap["icon"] = obj["weather"].toArray().first().toObject()["icon"].toString();
                        dailyForecastData.append(dataMap);
                    }
                ...
                {
                    QMessageBox::warning(this, "error", "无法获取天气数据：" + reply->errorString());
                }
                reply->deleteLater();
            });
        }
        ...
        {
            QMessageBox::warning(this, "错误", "无法获取地理坐标：" + coordReply->errorString());
        }
...
void WeatherWidget::updateUI(Ui::MainWindow *ui)
{
    ...
    connect(iconReply, &QNetworkReply::finished, this, [this, iconReply, ui]() {
        ...
        {
            QMessageBox::warning(this, "错误", "无法加载天气图标：" + iconReply->errorString());
        }
...
void WeatherWidget::fetchAlertsFromWeatherAPI(const QString &cityName, const QString &weatherApiKey)
{
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply]() {
        ...
        {
            QMessageBox::warning(this, "错误", "无法获取预警数据：" + reply->errorString());
        }
        reply->deleteLater();
    });
}
